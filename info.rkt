...
(define matching          #(128 119 168))

(define unused-color #(168 117 123))

(define colors `(; Colors
                 (drracket:syncheck:document-identifier    ,background)
                 (drracket:syncheck:matching-identifiers   ,matching)
                 ...
                 (drracket:check-syntax:lexically-bound             ,name)
                 (drracket:check-syntax:my-obligation-style-pref    ,name)
                 ...
                 (framework:syntax-color:scheme:symbol              ,name)
